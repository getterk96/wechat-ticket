...
class WeChatHandler(object):
    ...
    def get_activity(self):
        try:
            input = self.input['EventKey'][17:]
            activities = Activity.objects.filter(id=input, status=1)
        except:
            input = self.input['Content'].split()[-1]
            activities = Activity.objects.filter(name=input, status=1)

        if activities.exists():
            return activities
        return None

    def get_activity_atomic(self):
        try:
            input = self.input['EventKey'][17:]
            activities = Activity.objects.filter(id=input, status=1).select_for_update()
        except:
            input = self.input['Content'].split()[-1]
            activities = Activity.objects.filter(name=input, status=1).select_for_update()
    ...
    def is_get_ticket_text(self, texts):
        return self.is_msg_type('text') and (texts in self.input['Content'].lower())
    ...
    def is_get_ticket_click(self):
        return self.is_msg_type('event') and (self.input['Event'] == 'CLICK') and \
               (self.input['EventKey'][0:17] == 'BOOKING_ACTIVITY_')
    ...
    def url_activity(self, params):
        return settings.get_url('u/activity', params)

    def url_ticket(self, params):
        return settings.get_url('u/ticket', params)
